...
def ensure_project_venv() -> None:
    def choose_cache_dir(preferred: Path, suffix: str) -> Path:
        try:
            ...
            fallback = Path(tempfile.gettempdir()) / f"nac_fchk_matrix_{suffix}"
            fallback.mkdir(parents=True, exist_ok=True)
            return fallback

    mpl_cache_dir = choose_cache_dir(MPL_CACHE_DIR, "mpl_cache")
    fontconfig_cache_dir = choose_cache_dir(FONTCONFIG_CACHE_DIR, "fontconfig_cache")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Stampa sanity check numerici e genera la heatmap atomo×modo.",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=5,
        help="Numero di modi dominanti da stampare in classifica. Default: 5.",
    )
    parser.add_argument(
        "paths",
        nargs="*",
...
def write_mode_summary_tsv(
    output_path: Path,
    frequencies: list[float],
    reduced_masses: list[float],
    dq1_modes: list[float],
    dq1_sq: list[float],
) -> Path:
    lines = ["mode\tfreq_cm-1\tmu_amu\tdQ1_fcclasses\t|dQ1|^2"]
    for mode_index, (frequency, reduced_mass, dq1_mode, dq1_mode_sq) in enumerate(
        zip(frequencies, reduced_masses, dq1_modes, dq1_sq),
        start=1,
    ):
        lines.append(
            f"{mode_index}\t{frequency:.10f}\t{reduced_mass:.10f}\t{dq1_mode:.12e}\t{dq1_mode_sq:.12e}"
        )
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return output_path


def write_atom_mode_matrix_tsv(
    output_path: Path,
    atomic_numbers: list[int],
    atom_mode_matrix_q: list[list[float]],
) -> Path:
    header = ["atom_index", "atom_symbol"] + [f"m{mode_index + 1}" for mode_index in range(len(atom_mode_matrix_q[0]))]
    lines = ["\t".join(header)]
    for atom_index, row in enumerate(atom_mode_matrix_q, start=1):
        symbol = atom_symbol(atomic_numbers[atom_index - 1])
        values = "\t".join(f"{value:.12e}" for value in row)
        lines.append(f"{atom_index}\t{symbol}\t{values}")
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return output_path


def rank_modes_by_intensity(
    frequencies: list[float],
    reduced_masses: list[float],
    dq1_modes: list[float],
    dq1_sq: list[float],
    ...
    ranked = [
        (mode_index, frequency, reduced_mass, dq1_mode, dq1_mode_sq)
        for mode_index, (frequency, reduced_mass, dq1_mode, dq1_mode_sq) in enumerate(
            zip(frequencies, reduced_masses, dq1_modes, dq1_sq),
            start=1,
        )
...
def print_report(
    ...
    reduced_masses: list[float],
    nac_cart: list[float],
    coord_masses_amu: list[float],
    coord_masses_au: list[float],
    cartesian_modes: list[list[float]],
    l_columns: list[list[float]],
    atom_mode_matrix_q: list[list[float]],
    ...
    dq1_fcclasses = [sum(column) for column in mode_columns]
    dq1_sq = [value * value for value in dq1_fcclasses]
    modal_vector_norm_q = norm(dq1_fcclasses)
    atom_mode_frobenius_norm_q = math.sqrt(sum(value * value for row in atom_mode_matrix_q for value in row))
    cart_norm = norm(nac_cart)
    cart_massweighted_norm = mass_weighted_norm(nac_cart, coord_masses_au)
    residual_nonvib_sq = max(cart_massweighted_norm * cart_massweighted_norm - modal_vector_norm_q * modal_vector_norm_q, 0.0)
    residual_nonvib_norm = math.sqrt(residual_nonvib_sq)
    ctci_deviation = max_deviation_from_identity(cartesian_modes)
    ctmci_mu_deviation = max_deviation_from_mass_relation(cartesian_modes, coord_masses_amu, reduced_masses)
    ...
    top_n = max(1, min(top_n, len(frequencies)))
    ranked_modes = rank_modes_by_intensity(frequencies, reduced_masses, dq1_fcclasses, dq1_sq)
    ...
    spectrum_sq_png, spectrum_sq_pdf = write_mod_spectrum_plot(
        plot_dir / f"{prefix}_spectrum_dq1_sq",
        frequencies,
        dq1_sq,
    ...
    mode_summary_tsv = write_mode_summary_tsv(
        plot_dir / f"{prefix}_mode_summary.tsv",
        frequencies,
        reduced_masses,
        dq1_fcclasses,
        dq1_sq,
    )
    atom_mode_matrix_tsv = write_atom_mode_matrix_tsv(
        plot_dir / f"{prefix}_atom_mode_matrix.tsv",
        atomic_numbers,
        atom_mode_matrix_q,
    )
    ...
    print(f"    norma_cartesiana_massweighted_au={format_float(cart_massweighted_norm)}")
    print(f"    norma_vettore_modale_dQ1={format_float(modal_vector_norm_q)}")
    print(f"  top_{top_n}_modi_per_|dQ1|^2:")
    print("    rank  mode      freq_cm-1        mu_amu    dQ1_fcclasses           |dQ1|^2")
    for rank, (mode_index, frequency, reduced_mass, dq1_mode, dq1_mode_sq) in enumerate(ranked_modes[:top_n], start=1):
        print(
            "    "
            f"{rank:>4d}"
            f"{mode_index:>6d}"
            ...
            f"{dq1_mode:>17.12f}"
            f"{dq1_mode_sq:>17.12f}"
        )
    print(f"  plot_spectrum_sq_png={spectrum_sq_png}")
    print(f"  plot_spectrum_sq_pdf={spectrum_sq_pdf}")
    print(f"  mode_summary_tsv={mode_summary_tsv}")
    print(f"  atom_mode_matrix_tsv={atom_mode_matrix_tsv}")

    print("  riepilogo modi completo:")
    print("    mode      freq_cm-1        mu_amu    dQ1_fcclasses           |dQ1|^2")
    for mode_index, (frequency, reduced_mass, dq1_mode, dq1_mode_sq) in enumerate(
        zip(frequencies, reduced_masses, dq1_fcclasses, dq1_sq),
        start=1,
    ):
        print(
            "    "
    ...
    if debug:
        ...
        print("  debug_checks:")
        print(f"    residuo_nonvib_massweighted_au={format_float(residual_nonvib_norm)}")
        print(f"    norma_frobenius_matrice_atom_mode={format_float(atom_mode_frobenius_norm_q)}")
...
#            continue
        ...
        coord_masses_amu = repeat_masses_per_coordinate(vib_at_mass)
        coord_masses_au = [mass * AMU_TO_AU for mass in coord_masses_amu]
        l_columns = build_l_columns(cartesian_modes, reduced_masses, coord_masses_amu)
        atom_mode_matrix_q = build_atom_mode_matrix_q(nac_cart, l_columns, coord_masses_au, n_atoms)
